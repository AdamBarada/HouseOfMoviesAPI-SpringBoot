...
$(document).ready(function() {
    ...
    if(token){
        $.ajax({
            url: 'http://localhost:8080/admin/request/reservations',
            type: 'GET',
...
function pageLoaded(){
    ...
    list = '<tr><td>'+res[i]["id"]+'</td><td>'+res[i]["user"]["firstName"]+' '+res[i]["user"]["lastName"] 
        +'</td><td>'+res[i]["screening"]["movie"]["title"]+'</td><td>'
        + res[i]["date"] +' ' + res[i]["time"] +'</td><td>'+res[i]["screening"]["date"] + ' '+ res[i]["screening"]["time"] 
