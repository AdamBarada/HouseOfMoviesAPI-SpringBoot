...
$(document).ready(function() {
    ...
    if(token){
        $.ajax({
            url: 'http://localhost:8080/admin/request/screenings',
            type: 'GET',
...
function pageLoaded(){
    ...
    list = '<tr><td>'+res[i]["id"]+'</td><td>'+res[i]["movie"]["title"] 
        +'</td><td>'+res[i]["date"]+'</td><td>'+ screeningTime +'</td><td>'+res[i]["room"]["name"]+'</td>';
    if(res[i]["status"] == "AVAILABLE"){
        list += '<td><span class="mode mode_on">Upcoming</span></td>';
    } else if(res[i]["status"] == "STARTED"){
        list += '<td><span class="mode mode_done">Started</span></td>';
    } else list += '<td><span class="mode mode_off">Past</span></td>';
